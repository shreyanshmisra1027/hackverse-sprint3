...
export class P2PChat {
  ...
  send(text: string) {
    if (this.channel?.readyState !== 'open') throw new Error('The private connection is not ready yet.')
  ...
  private async handleSignal(data: unknown) {
    ...
    if (message.type === 'PEER_JOINED') {
      ...
      this.ensurePeer()
      this.handlers.onState('connecting', 'Negotiating a private connection.')
  ...
  private ensurePeer() {
    ...
    peer.addEventListener('connectionstatechange', () => {
      if (peer.connectionState === 'failed') this.handlers.onState('error', 'Could not establish a private connection.')
    })
    this.peer = peer
    return peer
  }
