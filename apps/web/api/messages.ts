...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  try {
    ...
    if (request.method === "POST") {
      const { roomId = "", recipientUsername = "", text = "" } = request.body ?? {};
      ...
      await database().query(
        `INSERT INTO message_archive (id, sender_id, recipient_username, room_id, ciphertext, iv, auth_tag)
         VALUES ($1, $2, $3, $4, $5, $6, $7)`,
        [randomUUID(), user.id, recipientUsername, roomId, encrypted.ciphertext, encrypted.iv, encrypted.authTag]
      );
      return json(response, 201, { saved: true });
    }
    const result = await database().query(
      `SELECT id, recipient_username, room_id, ciphertext, iv, auth_tag, created_at
       FROM message_archive WHERE sender_id = $1 ORDER BY created_at DESC LIMIT 100`, [user.id]
    );
    return json(response, 200, { messages: result.rows.map((row) => ({ id: row.id, recipientUsername: row.recipient_username, roomId: row.room_id, text: decryptMessage(row), createdAt: row.created_at })) });
  } catch (error) {
